...
A simple build script to create a Lambda layer for [Sharp](https://sharp.pixelplumbing.com/) with support for [HEIF](https://en.wikipedia.org/wiki/High_Efficiency_Image_File_Format) files. In order to run the build, [Docker](https://www.docker.com/) needs to be installed on your system.

To create the Lambda layer artifact, simply run:

```
./run.sh
```

It will create a pre-tested zip file. The zip file can be easily integrated into IaC pipelines or used manually.
...
The default script targets the `x86_64` platform. To build for `arm64`, run:
