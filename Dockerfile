...
RUN curl -L https://storage.googleapis.com/aom-releases/libaom-${LIBAOM_VERSION}.tar.gz | tar zx && \
    cd libaom-${LIBAOM_VERSION} && cd build && cmake -DCONFIG_AV1_ENCODER=0 -DBUILD_SHARED_LIBS=1 -DENABLE_TESTS=0 -DENABLE_DOCS=0 -DCMAKE_INSTALL_PREFIX=${PREFIX_PATH} -DCMAKE_INSTALL_LIBDIR=${PREFIX_PATH}/lib .. && make V=1 && make install
...
RUN curl -L https://github.com/libvips/libvips/archive/refs/tags/v${VIPS_VERSION}.tar.gz | tar zx && \
    cd libvips-${VIPS_VERSION} && meson setup build --prefix ${PREFIX_PATH} --libdir ${PREFIX_PATH}/lib -Dheif=enabled -Dheif-module=disabled --buildtype release && \
    cd build && meson compile && meson install

# install sharp
RUN mkdir -p ${PREFIX_PATH}/nodejs && mkdir -p ${PREFIX_PATH}/sharp-lib && \
    npm install --prefix ${PREFIX_PATH}/nodejs node-addon-api node-gyp && npm --prefix ${PREFIX_PATH}/nodejs install --cpu=x64 --os=linux --verbose --foreground-scripts sharp@${SHARP_VERSION} && \
